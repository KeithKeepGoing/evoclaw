...
**v1.10.27** — **5 項修正**：`main.py` 速率限制 deque 加上 `maxlen` 上限防止記憶體無限增長（#118）、`ipc_watcher.py` 新增 `_cleanup_stale_results()` 定期清除超過 1 小時的孤立 result 檔案防磁碟爆滿（#119）、`immune.py` 區分暫時性 DB 鎖（fail-open）與永久性 DB 錯誤（fail-secure）防止 prune_old_logs 短暫鎖定時全群組訊息被封鎖（#120）、`main.py` graceful shutdown 新增明確取消所有 asyncio 任務讓 SIGTERM 後立即退出不等待 sleep 到期（#121）、`task_scheduler.py` 無效排程表達式現在將任務標記為 `status=paused` 並記錄原因而非靜默留下 `next_run=NULL`（#122）。**v1.10.26** — **5 項修正**：`evolution/daemon.py` `_sync_prune_logs()` 加入 300 秒超時防止演化 daemon 掛死（#112）、`ipc_watcher.py` `_sanitize_error_for_notification()` 改用雙 regex 正確移除 Windows 和 Unix 絕對路徑（#113）、`ipc_watcher.py` subagent result 截斷改用 byte-aware 方式防止多位元組 UTF-8 字元被截斷（#114）、`task_scheduler.py` 群組找不到時改為套用 1 小時 backoff 而非永久停用（#115）、`main.py` 群組失敗計數冷卻後衰減 -2 而非歸零（#116）。**v1.10.25** — **6 項修正**：`main.py` `_is_duplicate_message()` TOCTOU 競態條件修正（全部 check+insert 包入單一 `async with _dedup_lock:`，#105）、`task_scheduler.py` `run_task()` 在 `finally` 區塊中必定推進 `next_run`（防止非預期例外後任務卡在過去時間戳，#106）、`webportal.py` `_pending_replies` 改為儲存 `(session_id, created_at)` tuple 並新增 300 秒 TTL 驅逐（#107）、`immune.py` `check_message()` 改為 fail-secure（DB 錯誤時回傳 `False` 拒絕而非 `True` 放行，#108）、`ipc_watcher.py` skill IPC 操作加入 300 秒超時防止 `_skills_lock` 無限持鎖（#109）、`container_runner.py` 所有 container stderr 行寫入 log 前通過 `_redact_secrets()` 防止 API 金鑰洩漏至日誌（#110）。**v1.10.24** — **2 項修正**：`dev_engine.py` Stage 7 路徑穿越防護改用 `Path.relative_to()` 取代字串 `startswith()` 防止 `/base_evil/file` 誤判（#92）、`webportal.py` 將 `_pending_replies[msg_id] = session_id` 移入 `_sessions_lock` 鎖內消除並發 `/api/send` 請求的競態條件（#90）。**v1.10.23** — **9 項修正**：router 所有分塊傳送失敗時通知用戶（#86）、Discord `future.result(30)` 加入 TimeoutError 捕捉防崩潰（#87）、WhatsApp `_last_wamid` 改用 LRU OrderedDict 上限 10,000 項防記憶體無限增長（#88）、`fitness.py` `speed_score` 公式修正（低於目標回應時間正確計為 1.0，#89）、WebPortal sessions 鎖在 `db.store_message()` 前釋放防 deadlock（#90）、Telegram 上傳逾時改由 `TELEGRAM_UPLOAD_TIMEOUT` 環境變數設定（預設 300s，原硬編碼 120s，#91）、DevEngine 路徑穿越防護加強（#92）、`immune.py` `check_message()` 新增空 `sender_jid` 守衛（#93）。**v1.10.22** — **WhatsApp `send_typing` wamid 修正**（#66）、`send_file` IPC 新增 `deleteAfterSend` 旗標（#68）、所有 LLM provider 支援逗號分隔多金鑰輪換（#6）、正式關閉 per-JID 時間戳游標問題（#5）。**v1.10.21** — **Docker 生產就緒鏡像**：升級基礎鏡像至 `node:22`（完整 Debian）提升系統庫相容性（#83）、預裝完整文件生成套件 `reportlab`/`openpyxl`/`python-docx`（PDF/Excel/Word，#77）、預裝網頁爬蟲套件 `httpx`/`beautifulsoup4`/`lxml`（#78）、預裝圖像處理 `Pillow`（#79）、預裝數據分析 `pandas`/`numpy`/`matplotlib`（#80）、完整 CJK 字型覆蓋含 `fonts-liberation`/`fonts-noto-color-emoji`（#81）、新增系統工具 `wget`/`unzip`/`jq`/`ffmpeg`（#82）、加入 Python 原生擴展構建工具 `python3-dev`/`build-essential`/`gcc`、`apt-get install` 改用 `--no-install-recommends` 保持鏡像精簡。**v1.10.20** — **第八輪 PUA 深度分析修正**：Gmail 郵件 body 新增 32 KB 上限防止 LLM context window 爆滿（#69）、Telegram 非文字訊息新增友善回覆而非靜默忽略（#70）、GroupQueue 所有 `create_task()` 補齊 done-callback 防止例外被靜默吞噬（#71）、`.env.example` 補齊所有缺失設定項含安全關鍵 `WHATSAPP_APP_SECRET`（#72）、IPC watcher 所有 `ensure_future()` 補齊 done-callback（#73）、Discord `disconnect()` 修正跨 event loop 呼叫 `client.close()` 導致 deadlock（#67）。**v1.10.18** — **第七輪 PUA 深度分析修正**：container name 改用 UUID 前 8 字元防止同秒碰撞（#59）、5 個 DB 讀取函式補齊 `_db_lock` 防 SQLITE_LOCKED（#60）、`docker run` 新增 `--memory`/`--cpus` 上限防 OOM（#61）、演化 daemon 每 24h 執行 `PRAGMA wal_checkpoint(TRUNCATE)` 防 WAL 無限膨脹（#62）、移除 `prune_old_logs` 中未使用的 `immune_cutoff_ms` 死碼（#63）、`init_database()` 新增 `PRAGMA foreign_keys = ON`（#64）。**v1.10.17** — **第六輪 PUA 深度分析修正**：per-JID 訊息游標（修正全域 `_last_timestamp` 導致多群組訊息靜默丟失）、`get_new_messages` / `get_conversation_history` 補齊 `_db_lock`（修正 dashboard 執行緒競態讀取）、task_scheduler 例外路徑補齊 `next_run` 更新（防止任務失敗後緊密無限重試）、`_get_empty_env_file()` 補齊 `threading.Lock`（防止並發首次呼叫建立重複 temp file）、SSE log stream 新增 `_dashboard_stopping` 旗標（shutdown 時立即退出不等待 client 斷線）、subagent result file 新增 1 MB 上限（防止 runaway subagent 填滿磁碟）、scheduler 空 `chat_jid` 守衛（防止以空字串為 key 汙染 GroupQueue）。**v1.10.16** — **安全強化 + 執行緒安全 + 頻道修正**：WhatsApp webhook 新增 HMAC-SHA256 簽名驗證（防止偽造 payload）、WebPortal POST 端點新增 CSRF token 防護、immune.py 指紋雜湊從 MD5 升級至 SHA-256；12 個 DB 讀取函式補齊 `_db_lock`（防止 dashboard/webportal 執行緒競態）、Discord 頻道修正跨 event loop 呼叫問題（使用 `asyncio.run_coroutine_threadsafe`）、Gmail `_seen_message_ids` 改用有上限的 LRU OrderedDict（防止記憶體無限增長）、Slack 將 `auth_test()` 快取於連線時（不再每則訊息都呼叫 API）、IPC 錯誤通知sanitize 內部路徑（防止洩漏目錄結構）。**v1.10.15** — **research-ppt skill 自癒架構 + Telegram 檔案發送修正**：新增 `research-ppt` skill，提供版本鎖定（`python-pptx==1.0.2`）、自癒安裝重試（最多 2 次）、優雅降級（PPTX 失敗自動轉純文字 .txt）、CJK 字型安全（找不到字型不崩潰）；`route_file()` 新增 45 MB 檔案大小護衛與存在性檢查；`TelegramChannel.send_file()` 改用串流（不再一次讀入記憶體）、新增 120 秒上傳逾時防止 GroupQueue 卡住、移除 debug_send.log 副作用。**v1.10.14** — **第四輪可靠性與邊界案例修正**：免疫系統 `record_immune_threat()` DB 競態條件修正（補齊 `_db_lock`）、`prune_old_logs()` 擴展至清理所有資料表（`evolution_log`、`messages`、`immune_threats`、`dev_events`、`dev_sessions`）、演化 daemon 每 24 小時自動執行一次 DB 清理（不再只有啟動時）、`psutil` 加入 `requirements.txt`（修正 `health_monitor` 啟動 ImportError）、實作 `db.get_pending_task_count()` 和 `db.get_error_stats()` 讓健康監控真正運作、LLM API 呼叫（Gemini / Claude / OpenAI）新增指數退避重試（最多 3 次，針對 429 / 5xx 錯誤）。**v1.10.13** — **第三輪安全性與可靠性修正**：agent 檔案工具路徑沙盒（防止讀取 `/proc/self/environ` 等敏感容器路徑）、Skills `post_apply` 指令白名單（防止惡意 skill manifest 執行任意主機指令）、IPC 路徑穿越防護、WebPortal `_pending_replies` 記憶體洩漏修正、DB 寫入函式補齊 `_db_lock`（9 個函式）、WebPortal 繞過速率限制漏洞修正、訊息分段發送失敗通知用戶、WebPortal session 數量上限（500）與 POST body 大小限制（64KB）、`ENABLED_CHANNELS` 啟動驗證。**v1.10.12** — **安全性與穩定性**：WebPortal Basic Auth 防止未授權存取、per-group 訊息速率限制（20 msgs/60s 可設定）、GroupQueue 背壓上限（pending_tasks ≤ 50）、SQLite 寫入加 threading.Lock 防競態、修正 fitness speed_score 對失敗 run 的計算錯誤、日誌表 30 天自動清理防磁碟爆滿、`LOG_FORMAT=json` 結構化日誌支援、container image 未 pin 版本啟動警告。**v1.10.11** — **架構改善**：新增 `run_id` 關聯 ID 串接 host/container 日誌、訊息去重防止 webhook 重試重複處理、graceful shutdown 等待 container 完成、修正 IPC 未知 type 靜默忽略、修正 `_resolve_container_path` NameError、`asyncio.to_thread` 取代 deprecated `get_event_loop()`。**v1.10.10** — **穩定性修正**：JSON 輸出 2MB 上限防止 DoS、circuit breaker asyncio.Lock 競態條件修正、DB connection atexit 正確關閉、stderr readline 30s 超時、Secret key 啟動驗證、group folder 路徑穿越漏洞修正、孤立任務清理完整化。**v1.10.9** — **對話記憶改善**：移除 800 字截斷、Session 管理修正（newSessionId 改用 UUID）、歷史時間窗可設定（預設 4 小時）、歷史訊息上限提升至 50 則。**v1.10.8** — **動態容器工具熱插拔（Skills 2.0）**：DevEngine 生成的 Skill 現可新增 Python 工具至容器，不需重建 image。`data/dynamic_tools/` 掛載至 `/app/dynamic_tools:ro`，啟動時自動 import 並注冊至 Gemini / Claude / OpenAI 三大 provider；Skills manifest 新增 `container_tools:` 欄位。**v1.10.7** — 修復 Telegram 二進位檔案發送 `cp950` 編碼錯誤。
